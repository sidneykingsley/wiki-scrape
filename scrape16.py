...
import csv
folder_name = 'results'
file_name = 'wiki-o3.txt'
count = 1
with open('names/olympic-medalists.csv', newline='') as f:
    reader = csv.reader(f)
    names = list(reader)
...
for x in names:
    ...
    try:
        ...
        app_results = []
        try:
            for main_cont in soup.find("div", {"class":"mw-parser-output"}).findAll('table', {"class":"infobox vcard"}):
                mc_node = main_cont
                while True:
